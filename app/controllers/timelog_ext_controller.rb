class TimelogExtController < ApplicationController
  menu_item :issues
  ...
  def report
    ...
    unless @criterias.empty?
      ...
      sql << " COALESCE(rates.amount, 0) as rate_amount, COALESCE(rates.amount, 0) * SUM(hours) as cost"
      sql << " FROM #{TimeEntry.table_name}"
      sql << " LEFT JOIN #{Issue.table_name} ON #{TimeEntry.table_name}.issue_id = #{Issue.table_name}.id"
      sql << " LEFT JOIN #{Project.table_name} ON #{TimeEntry.table_name}.project_id = #{Project.table_name}.id"
      sql << " LEFT JOIN rates ON rates.project_id = #{TimeEntry.table_name}.project_id and rates.user_id = #{TimeEntry.table_name}.user_id"
      sql << " WHERE"
      ...
      sql << " GROUP BY #{sql_group_by}, tyear, tmonth, tweek, spent_on, rate_amount"
